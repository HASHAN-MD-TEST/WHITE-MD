...
if (!fs.existsSync(__dirname + '/session/creds.json')) {
    if (!config.SESSION_ID) return console.log("Please Add SESSION_ID ➾")
      const sessdata = config.SESSION_ID.split("HASHAN-MD=")[1];
...
async function connectToWA() {;
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('HASHAN MD Plugins installed ✅')
            console.log('HASHAN MD Bot connected ✅')
...
let up = "HASHAN-MD BOT CONNECTED SUCCESSFULL\n\nPrefix :-" + prefix + "\nMode :- " + mode + "\nStatus Read :-" + statusRead + "\n\n> ᴘᴀᴡᴇʀᴇᴅ ʙʏ ꜱᴜᴘᴜɴ ᴍᴅ";
...
if (metadata.viewer_metadata === null) {
  await conn.newsletterFollow("120363402463833530@newsletter");
...
app.get("/", (req, res) => {
res.send("HASHAN MD CONNECTED SUCCESSFUL💀");
});
